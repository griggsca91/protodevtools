...
describe('removeGrpcEncoding', () => {
  it('works on requests', () => {
    const sayHelloRequestBase64 = readFileSync('./src/lib/testfiles/SayHelloRequest.grpc.txt', { encoding: 'ascii' })
  ...
  it('works on responses', () => {
    const sayHelloRequestBase64 = readFileSync('./src/lib/testfiles/SayHelloResponse.grpc.txt', { encoding: 'ascii' })
    const buf = Buffer.from(base64Decode(sayHelloRequestBase64))
    const cleanedBuf = removeGrpcEncoding(buf)
  ...
  it('works on larger than 255 byte responses', () => {
    const sayHelloRequestBase64 = readFileSync('./src/lib/testfiles/SayHelloResponseLarge.grpc.txt', { encoding: 'ascii' })
    const buf = Buffer.from(base64Decode(sayHelloRequestBase64))
    const cleanedBuf = removeGrpcEncoding(buf)
  ...
  it('ignores non-grpc requests', () => {
    const sayHelloRequestBase64 = readFileSync('./src/lib/testfiles/SayHelloRequest.proto.txt', { encoding: 'ascii' })
    const buf = Buffer.from(base64Decode(sayHelloRequestBase64))
    const cleanedBuf = removeGrpcEncoding(buf)
    assert.equal(cleanedBuf.length, 7)
