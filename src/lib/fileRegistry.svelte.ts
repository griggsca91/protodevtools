  ...
  openRequest.onsuccess = function () {
    ...
    request.onsuccess = function () {
      ...
      registries = request.result.map((fr) => {
        const fileDescriptorSet = fromBinary(FileDescriptorSetSchema, fr.data);
...
export default {
  get activeFileRegistry(): DevtoolFileRegistry | null {
    if (localStorage) {
      ...
      const foundRegistry =
        registries.find((r) => r.name == registryName) ?? null;
  ...
  addFileRegistryFromBinary(fileName: string, b: Uint8Array) {
    const fileDescriptorSet = fromBinary(FileDescriptorSetSchema, b);
    // Create a FileRegistry from the google.protobuf.FileDescriptorSet message:
